/*
 =================================================================================
  Stackswan LoRa p2p - (endpoint)
 ---------------------------------------------------------------------------------
  1) Endpoint can use any unique 2byte address otherthan 0xFF.
...
byte localAddress = 0xAA;     // address of this device(unique for each device)
byte destination = 0xFF;      // destination to send to(0xFF for all border routers)
...
void loop() {
  ...
  // This is only for endpoint devices
  if (millis() - lastSendTime > interval) {
    String message = "Hello from stackswan !";   //send a message
    sendMessage(message);
